...
public class Moonlight extends Spell {
    ...
    public boolean cast(World world, EntityLiving caster, EnumHand hand, int ticksInUse, EntityLivingBase target, SpellModifiers modifiers) {
        ...
        //check if the caster can be healed and if the moon is out and visible by the player
        if (caster.getHealth() < caster.getMaxHealth() && caster.getHealth() > 0.0F && ticksInUse % 10 == 0 && WNGUtils.hasMoonlight(caster)) {
    ...
    public void spawnParticles(World world, EntityLivingBase caster, SpellModifiers modifiers){
        for(int i = 0; i < 10; i++){
            ...
            ParticleBuilder.create(ParticleBuilder.Type.SPARKLE).pos(x, y, z).vel(0, 0.1, 0).clr(204, 255, 255).spawn(world);
        }
        ParticleBuilder.create(ParticleBuilder.Type.BUFF).entity(caster).clr(204, 255, 255).spawn(world);
    }
