...
public class ItemNewArtefact extends Item {
    ...
    public enum AdditionalType {
        ...
        /** An artefact that fits into the Baubles BODY slot. One of these can be active at any one time. */ BODY(1);
    ...
    public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable NBTTagCompound nbt){
        return WNGBaublesIntegration.enabled() ? new WNGBaublesIntegration.ArtefactBaubleProvider(type) : null;
    }
    ...
    public static List<ItemNewArtefact> getActiveNewArtefacts(EntityPlayer player, ItemNewArtefact.AdditionalType... types){
        ...
        if(WizardryBaublesIntegration.enabled() && WNGBaublesIntegration.enabled()){
            return WNGBaublesIntegration.getEquippedArtefacts(player, types);
        }else{
